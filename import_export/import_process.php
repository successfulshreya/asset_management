...
include 'config/config.php'; // Assumes $conn is defined here (mysqli)
...
if (isset($_POST['import']) && $_FILES['file']['size'] > 0) {
    ...
    while (($data = fgetcsv($file, 1000, ",")) !== FALSE) {
        // Extract and sanitize all relevant fields
        $user_name = $data[0];
        $designation = $data[1];
        $department = $data[2];
        $employee_id = $data[3];
        ...
        $laptop_adaptor_serial_number = $data[36];
        ...
        $amc = $data[41];
        $created_at = $data[42];
        $change_reason = ''; // You can modify this as needed
    ...
    fclose($file);
    echo "<script>alert('Import successful!'); window.location.href='asset_list.php';</script>";
} else {
